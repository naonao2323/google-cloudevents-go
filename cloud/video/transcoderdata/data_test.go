...
import (
	...
	"github.com/googleapis/google-cloudevents-go/cloud/video/transcoderdata"
...
func TestParsingJobEventData(t *testing.T) {
	cases := testhelper.FindTestData(t, "JobEventData", "google/events/cloud/video/transcoder/v1")
	...
	for name, file := range cases {
		t.Run(name, func(t *testing.T) {
			...
			t.Run("loose", func(t *testing.T) {
				out := transcoderdata.JobEventData{}
...
//   - no deleted or renamed fields in protos covered in test data
//   - test data does not carry unknown fields
func TestParsingJobTemplateEventData(t *testing.T) {
	cases := testhelper.FindTestData(t, "JobTemplateEventData", "google/events/cloud/video/transcoder/v1")
	...
	for name, file := range cases {
		t.Run(name, func(t *testing.T) {
			...
			t.Run("loose", func(t *testing.T) {
				out := transcoderdata.JobTemplateEventData{}
				pj := protojson.UnmarshalOptions{DiscardUnknown: true}
				if err := pj.Unmarshal(data, &out); err != nil {
					t.Fatalf("protojson.Unmarshal: could not parse %q\n----%s\n----", file, data)
				}
